	...
	if xPlayer ~= nil then
		if activeRobbery then
			if currentlyRobbing[src] then
				...
				print("^1esx_vangelico_robbery^0: ^4"..srcName.." (ID: "..tonumber(src)..")^0 is attempting to exploit the event: ^4esx_vangelico_robbery:recievejewels. This player is attempting to collect jewels but is not currently robbing a jewlery location!")
			end
		else
			--insert kick or ban event here
			--Kick: --DropPlayer(src, "Exploiting")
